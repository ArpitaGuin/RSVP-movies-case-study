...
select * from ct_avg_rating_rank
where movie_rank <= 10;
...
rank() over( order by round(sum(total_votes * avg_rating)/sum(total_votes), 2) desc) as actress_rank
from names n
inner join role_mapping rm
on n.id= rm.name_id
inner join ratings r
...
count(name) as movie_count, round(sum(avg_rating * total_votes)/sum(total_votes), 2) as actress_avg_rating,
dense_rank() over( order by count(name) desc) as actress_rank
from   names n
inner join role_mapping rm
on n.id= rm.name_id
inner join ratings r
